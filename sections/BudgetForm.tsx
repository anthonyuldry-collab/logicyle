    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                >
                    <option value="">Général (pas d'événement spécifique)</option>
                    {raceEvents?.map(event => (
                        <option key={event.id} value={event.id}>{event.name} - {new Date(event.date).toLocaleDateString('fr-FR')}</option>
                    ))}
                </select>
            </div>
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700">Coût réel (€) - optionnel</label>
                <input
                    type="number"
