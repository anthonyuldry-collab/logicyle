  ...
  const autoGeneratedTimings = useMemo(() => {
    ...
    const shouldShowInTiming = (leg: EventTransportLeg): boolean => {
        if (viewMode === 'individual' && selectedPersonId) {
            ...
            // Mode équipe : afficher les trajets collectifs ET les étapes individuelles importantes
            if (!isIndividualTrip(leg)) {
                return true; // Trajets collectifs toujours affichés
            } else {
                // Pour les trajets individuels, afficher seulement s'ils ont des étapes importantes
    ...
    appState.eventTransportLegs.filter(leg => leg.eventId === event.id).forEach(leg => {
        ...
        // Pour les trajets individuels, ne pas afficher les départs/arrivées principaux
        // mais seulement les étapes de récupération/dépose
        if (!isIndividual) {
            if (leg.departureDate && leg.departureTime) {
                const targetDay = getTargetMealDay(leg.departureDate);
                if (targetDay) {
                    // Informations sur le véhicule (simplifiées)
                    ...
                    let description = `🚗 Départ - ${vehicleInfo}${driverInfo}`;
            ...
            if (leg.arrivalDate && leg.arrivalTime) {
                ...
                if (targetDay) {
                    ...
                    // Créer une description détaillée pour le jour J
                    let description = `🏁 Arrivée - ${vehicleInfo}${driverInfo}`;
                    if (leg.arrivalLocation) {
                        description += ` - À: ${leg.arrivalLocation}`;
                    }
                    
                    // Ajouter les personnes récupérées si c'est un trajet avec récupérations
                    if (leg.occupants && leg.occupants.length > 0) {
                        const occupantsNames = leg.occupants.map(occ => getOccupantName(occ.id, occ.type)).join(', ');
                        description += ` - Récupère: ${occupantsNames}`;
                    }
