...
const EventOperationalLogisticsTab: React.FC<EventOperationalLogisticsTabProps> = ({ event, updateEvent, appState }) => {
  ...
  const getVehicleInfo = (vehicleId: string | undefined): string => {
    ...
    return vehicle ? `${vehicle.name} (${vehicle.seats} places)` : 'Véhicule inconnu';
  };

  const getDriverName = (driverId: string | undefined): string => {
    if (!driverId) return '';
    const driver = appState.staff.find(s => s.id === driverId);
    return driver ? ` - Conducteur: ${driver.firstName} ${driver.lastName}` : '';
  };
  ...
  const autoGeneratedTimings = useMemo(() => {
    ...
    appState.eventTransportLegs.filter(leg => leg.eventId === event.id).forEach(leg => {
        if (leg.departureDate && leg.departureTime) {
            ...
            if (targetDay) {
                // Informations sur le véhicule et les occupants
                ...
                const occupantsInfo = leg.occupants && leg.occupants.length > 0 
                    ? ` - Passagers: ${leg.occupants.map(occ => getOccupantName(occ.id, occ.type)).join(', ')}`
                    : '';

                timings.push({
                    id: `auto-transport-depart-${leg.id}`,
                    time: leg.departureTime,
                    description: `🚗 Départ ${leg.direction} - ${vehicleInfo}${driverInfo}${occupantsInfo} - De: ${leg.departureLocation || 'N/A'}`,
        ...
        if (leg.arrivalDate && leg.arrivalTime) {
            ...
            if (targetDay) {
                ...
                timings.push({
                    ...
                    description: `🏁 Arrivée ${leg.direction} - ${vehicleInfo}${driverInfo}${occupantsInfo} - À: ${leg.arrivalLocation || 'N/A'}`,
                    isAutoGenerated: true,
                    targetMealDay: targetDay,
                    category: OperationalTimingCategory.TRANSPORT
                });
        ...
        if (leg.intermediateStops && leg.intermediateStops.length > 0) {
            leg.intermediateStops.forEach(stop => {
                ...
                if (targetDay && stop.time) {
                    ...
                    if (isSimpleFormat) {
                        // Format simplifié pour les vols/trains
                        description = `${emoji} ${stopTypeLabel} ${stop.location}`;
                        if (personsConcerned) {
                            description += ` - Récupère: ${personsConcerned}`;
                        }
                        if (stop.isTimingCritical) {
                            description += ' ⏰';
                        }
                    } else {
                        // Format détaillé pour les autres étapes
                        description = `${emoji} ${stopTypeLabel} - ${vehicleInfo}${driverInfo}`;
                        
                        if (personsConcerned) {
                            description += ` - Personnes: ${personsConcerned}`;
                        }
                        
                        description += ` - Lieu: ${stop.location}`;
                        ...
                        if (stop.contactPerson) {
                            description += ` - Contact: ${stop.contactPerson}`;
                            if (stop.contactPhone) {
                                description += ` (${stop.contactPhone})`;
                            }
                        }
                        ...
                        if (stop.isPickupRequired) {
                            description += ' 🚨 RÉCUPÉRATION OBLIGATOIRE';
                        }
                        if (stop.isDropoffRequired) {
                            description += ' 🚨 DÉPOSE OBLIGATOIRE';
                        }
                        if (stop.reminderMinutes && stop.reminderMinutes > 0) {
                            description += ` 🔔 Rappel -${stop.reminderMinutes}min`;
                        }
                    }
    ...
    <div className="space-y-6">
      ...
      {isEditing ? (
        ...
        <>
          ...
          {(() => {
            const transportLegs = appState.eventTransportLegs.filter(leg => leg.eventId === event.id);
                ...
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <div className="space-y-3">
                    {transportLegs.map(leg => {
                      ...
                      const driverInfo = getDriverName(leg.driverId);
                      const occupantsInfo = leg.occupants && leg.occupants.length > 0 
                        ? leg.occupants.map(occ => getOccupantName(occ.id, occ.type)).join(', ')
                        : 'Aucun passager';
                      ...
                      return (
                        <div key={leg.id} className="bg-white p-3 rounded border border-blue-100">
                          ...
                          <div className="text-sm text-gray-600 space-y-1">
                            <div><strong>Conducteur:</strong> {driverInfo || 'Non assigné'}</div>
                  ...
                  return (
                    <li key={timing.id} className="flex items-start py-2 border-b border-gray-200 last:border-b-0">
                      ...
                      <div className="flex-1">
                        ...
                        {timing.category === OperationalTimingCategory.TRANSPORT && (
                          <div className="mt-1 text-xs text-gray-500">
                            {timing.isAutoGenerated ? '🔄 Généré automatiquement' : '✏️ Saisi manuellement'}
                          </div>
                        )}
                      </div>
                    </li>
